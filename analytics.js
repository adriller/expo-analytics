...
export default class Analytics {
    ...
    constructor(propertyId, additionalParameters = {}, options = defaultOptions) {
        ...
        this.promiseGetWebViewUserAgentAsync = getWebViewUserAgent(options).then((userAgent) => {
    ...
    send(hit) {
        ...
        const url = `https://www.google-analytics.com/collect?tid=${this.propertyId}&v=1&cid=${this.clientId}&${hit.toQueryString()}&${params}&${customDimensions}&${customMetrics}&z=${Math.round(
            Math.random() * 1e8
        )}`;
